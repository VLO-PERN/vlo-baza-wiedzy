...
const AdminEditArticle = (props: any) => {
    ...
    useEffect(() => {
        if (jwtContext.token === null) {
            ...
            fetch(`/api/v1/article/${id}`).then(res => res.json())
                .then(data => {
                    setText(data.article.contents);
                    setArticleName(data.article.name);
                    console.log(data);
                }).catch(err => {
                    console.error(err);
    ...
    return (<>
        ...
        <Bottom>
            <Input value={articleName} ref={inputName} onChange={changeArticleName} width="220px" marginLeft="20px" focusBorderColor="#004fff" color="#FFF" variant="outline" placeholder="Nazwa artykułu" />
            {isSaving ? <Spinner marginRight="25px" size="m" ></Spinner> :
                <Button onClick={sendEditToDb}>Zapisz</Button>
            }
        </Bottom>
    </>)
}
